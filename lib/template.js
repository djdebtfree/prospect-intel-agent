...
function generate(d) {
  ...
  // Pipeline Lite capabilities
  var plCapsHtml = (pl.capabilities || []).map(function(c) {
    return '<div class="capability-item"><strong>' + esc(c.title) + '</strong><p>' + esc(c.desc) + '</p></div>';
  }).join('');

  // Pipeline Lite KPIs
  var plKpisHtml = (pl.kpis || []).map(function(k, i) {
    return '<div class="kpi"><div class="val">' + esc(k.value) + (i === 0 ? '<span class="ghl-badge">Lead Connector HQ</span>' : '') + '</div><div class="label">' + esc(k.label) + '</div></div>';
  }).join('');

  // Data Driver integration discovery
  var ddDisc = dd.integrationDiscovery || {};
  var ddHighlightHtml = '<div class="highlight-box-green"><h3>' + (ddDisc.ghlDetected ? '&#10003; ' : '') + 'Integration Discovery</h3>' +
    '<p>' + esc(ddDisc.summary || '') + '</p>' +
    '<div style="display:flex;gap:20px;margin-top:12px;flex-wrap:wrap">' +
      '<div><span class="stat-label">Existing Tools</span><span class="stat-val">' + esc((ddDisc.existingTools || []).join(', ') || 'None detected') + '</span></div>' +
      '<div><span class="stat-label">Migration Required</span><span class="stat-val">' + esc(ddDisc.migrationRequired || 'N/A') + '</span></div>' +
    '</div></div>';

  // Data Driver capabilities
  var ddCapsHtml = (dd.capabilities || []).map(function(c) {
    return '<div class="capability-item"><strong>' + esc(c.title) + '</strong><p>' + esc(c.desc) + '</p></div>';
  }).join('');

  // Data Driver growth projections
  var ddProjHtml = (dd.growthProjections || []).map(function(p) {
    return '<div class="projection-row"><div class="projection-label">' + esc(p.label) + '</div><div class="projection-val up">' + esc(p.value) + '</div></div>';
  }).join('');

  // Data Driver integration path
  var ddPathHtml = (dd.integrationPath || []).map(function(p) {
    var effortClass = (p.effort || '').toLowerCase();
    return '<tr><td>' + esc(p.component) + '</td><td>' + esc(p.currentState) + '</td><td>' + esc(p.action) + '</td><td><span class="effort-badge ' + effortClass + '">' + esc(p.effort) + '</span></td></tr>';
  }).join('');
  ...
  return '<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>' + esc(d.businessName) + ' — Prospect Intelligence Dashboard</title><style>' +
    ...
    '.ghl-badge{background:rgba(34,197,94,.15);color:var(--green);padding:2px 8px;border-radius:8px;font-size:.65rem;font-weight:600;margin-left:6px}' +
    '.int-table{width:100%;border-collapse:collapse}' +
    '.int-table th{background:var(--surface2);text-align:left;padding:10px 12px;font-size:.8rem;color:var(--text2)}' +
    '.int-table td{padding:10px 12px;border-bottom:1px solid var(--border);font-size:.85rem}' +
    '.highlight-box-green{background:linear-gradient(135deg,rgba(34,197,94,.1),rgba(20,184,166,.1));border:1px solid rgba(34,197,94,.3);border-radius:12px;padding:20px;margin:16px 0}' +
    '.highlight-box-green h3{color:var(--green);font-size:1rem}' +
    '.highlight-box-green p{color:var(--text2);font-size:.9rem;margin-top:6px}' +
    '.capability-item{background:var(--surface2);border-radius:10px;padding:16px;margin:8px 0}' +
    '.capability-item strong{color:var(--accent);display:block;margin-bottom:4px}' +
    '.capability-item p{color:var(--text2);font-size:.85rem}' +
    '.effort-badge{padding:2px 10px;border-radius:12px;font-size:.75rem;font-weight:600}' +
    '.effort-badge.low{background:rgba(34,197,94,.15);color:var(--green)}' +
    '.effort-badge.medium{background:rgba(240,165,0,.15);color:var(--accent)}' +
    '.effort-badge.high{background:rgba(239,68,68,.15);color:var(--red)}' +
    ...
    '<div class="tabs">' +
      ...
      '<div class="tab" data-tab="pipeline">Pipeline Lite &amp; Data Driver</div>' +
    '</div>' +
    ...
    // Pipeline Lite & Data Driver Panel
    '<div class="panel" id="pipeline">' +
      ddHighlightHtml +
      '<div class="kpi-strip">' + plKpisHtml + '</div>' +
      '<div class="grid2"><div class="card"><h3 style="color:var(--accent)">Pipeline Lite Capabilities</h3>' + plCapsHtml + '</div>' +
      '<div class="card"><h3 style="color:var(--teal)">Data Driver Capabilities</h3>' + ddCapsHtml + '</div></div>' +
      '<div class="card"><h3>Growth Projections</h3>' + ddProjHtml + '</div>' +
      '<div class="card"><h3>Integration Path</h3><table class="int-table"><thead><tr><th>Component</th><th>Current State</th><th>Action</th><th>Effort</th></tr></thead><tbody>' + ddPathHtml + '</tbody></table></div>' +
      '<div class="card"><h3>Implementation Phases</h3>' + plPhasesHtml + '</div>' +
      '<div class="card"><h3>Projected Impact</h3>' + plProjHtml + '</div>' +
      '<div class="highlight-box"><h3>Pipeline Lite by Join Remote Sales</h3><p>AI-powered sales pipeline with Sandy Beach, your dedicated AI sales assistant. From lead research to closed deals — Pipeline Lite handles prospecting, outreach, follow-ups, and scheduling so your team can focus on closing.</p></div>' +
    '</div>' +
